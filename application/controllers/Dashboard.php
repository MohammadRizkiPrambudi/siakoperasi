...
class Dashboard extends CI_Controller
{
	...
	public function anggota_add()
	{
		if ($p = $this->input->post()) {
			$data = [
				...
				'anggota_nokk'		=> ucwords($p['anggota_nokk']),
	...
	{
		$cek = $this->data->cek_pinjaman($id)->result();
		foreach ($cek as $c) {
			if ($cek->total > $cek->pinjaman_tempo) {
				$this->db->delete('anggota', ['anggota_kode' => $id]);
				$this->session->set_flashdata('msg', "Toast.fire({ icon: 'success', title: 'Data berhasil dihapus' });");
				redirect('anggota');
			}
			$this->session->set_flashdata('msg', "Toast.fire({ icon: 'warning', title: 'Pinjaman belum lunas' });");
			redirect('anggota');
		}
	}
	...
	{
		$data['angsuran']	= 'active';
		$data['pinjaman'] = $this->data->cek_kekurangan($id)->row();
	...
	public function laporan_anggota($aksi = null)
	{
		if ($aksi) {
			...
			$pdf->AddPage();
			$pdf->Image(base_url('src/img/logo.png'), 10, 1, 29);
	...
	public function laporan_simpananp($aksi = null)
	{
		if ($aksi) {
			...
			$pdf->AddPage();
			$pdf->Image(base_url('src/img/logo.png'), 10, 5, 25);
			...
			$data['laporan']	= 'active';
			$data['lsimpanan']	= $this->data->anggota()->result();
	...
	public function laporan_pinjaman($aksi = null)
	{
		if ($aksi) {
			...
			$pdf 	= new FPDF('P', 'mm', 'A4');
			$pdf->AddPage();
			$pdf->Image(base_url('src/img/logo.png'), 10, 4, 25);
	...
	public function kartu_pinjaman($aksi)
	{
		...
		$pdf->Cell(40, 5, 'Kode Pinjaman', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->pinjaman_kode, 0, 1);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(40, 5, 'No. Handphone', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->anggota_nohp, 0, 1);
		$pdf->Cell(40, 5, 'Pekerjaan', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->anggota_pekerjaan, 0, 1);
		$pdf->Cell(40, 5, 'Alamat', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->anggota_alamat, 0, 1);
		$pdf->Cell(40, 5, 'Jumlah Pinjaman', 0, 0);
		$pdf->Cell(150, 5, ': Rp. ' . number_format($data->pinjaman_jumlah), 0, 1);
		$pdf->Cell(40, 5, 'Tempo Pinjaman', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->pinjaman_tempo . ' Bulan', 0, 1);
		$pdf->Cell(40, 5, 'Bunga Pinjaman', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->pinjaman_bunga . '%', 0, 1);
		$pdf->Cell(40, 5, 'Total Pengembalian', 0, 0);
		$pdf->Cell(150, 5, ': Rp. ' . number_format($data->pinjaman_total), 0, 1);
		$pdf->Cell(40, 5, 'Jaminan', 0, 0);
		$pdf->Cell(150, 5, ': ' . $data->pinjaman_jaminan, 0, 1);
	...
	public function laporan_angsuran($aksi = null)
	{
		if ($aksi) {
			...
			if ($data) {
				foreach ($data as $d) {
					$pdf->Cell(10, 6, $no++, 1, 0, 'C');
					$pdf->Cell(40, 6, $d->anggota_nama, 1, 0);
					$pdf->Cell(10, 6, $d->angsuran_ke, 1, 0, 'C');
					$pdf->Cell(30, 6, 'Rp. ' . number_format($d->angsuran_jumlah), 1, 0, 'C');
					$pdf->Cell(30, 6, 'Rp. ' . number_format($d->angsuran_denda), 1, 0, 'C');
					$pdf->Cell(30, 6, 'Rp. ' . number_format($d->angsuran_total), 1, 0, 'C');
					$bulan = explode('-', date('Y-m-d', strtotime($d->angsuran_waktu)));
					if ($bulan[2] > 10) {
						$pdf->Cell(40, 6, 'Terlambat', 1, 1, 'C');
					} else {
	...
	public function print_neraca($aksi = null)
	{
		if ($aksi) {
			$neraca		= $this->db->query("
				SELECT 
	...
	public function bukti_anggota($aksi)
	{
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Kode Anggota', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_kode, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'NIK Anggota', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_nik, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'No. Handphone', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_nohp, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Pekerjaan', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_pekerjaan, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Alamat', 0, 0);
		$pdf->Cell(100, 5, ': ' . $data->anggota_alamat, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Tanggal Masuk Anggota', 0, 0);
		$pdf->Cell(100, 5, ': ' . tanggal($data->tgl_masuk_anggota), 0, 1);
	...
	public function bukti_simpananwajib($aksi)
	{
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Kode Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->simpanan_kode, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Tanggal Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': ' . tanggal(date('Y-m-d', strtotime($data->simpanan_waktu))), 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jenis Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': Simpanan Wajib', 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jumlah', 0, 0);
		$pdf->Cell(110, 5, ': Rp.' . number_format($data->simpanan_jumlah), 0, 1);
	...
	public function bukti_simpanansukarela($aksi)
	{
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jenis Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': Simpanan Sukarela', 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
	...
	public function bukti_simpananpokok($aksi)
	{
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jenis Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': Simpanan Pokok', 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
	...
	public function bukti_penarikan($aksi)
	{
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Kode Penarikan', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->kode, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Tanggal Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': ' . tanggal(date('Y-m-d', strtotime($data->created_at))), 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jumlah', 0, 0);
		$pdf->Cell(110, 5, ': Rp.' . number_format($data->amount), 0, 1);
	...
	public function print_shup($aksi = null)
	{
		if ($aksi) {
			...
			$pdf->SetFont('times', 'B', 12);
			$pdf->Cell(55, 6, 'No Anggota', 1, 0, 'C');
			$pdf->Cell(55, 6, 'Nama Anggota', 1, 0, 'C');
			$pdf->Cell(55, 6, 'SHU Simpanan', 1, 0, 'C');
			$pdf->Cell(55, 6, 'SHU Pinjaman', 1, 0, 'C');
			...
			if ($data) {
				...
				foreach ($data as $d) :
					...
					$pdf->Cell(55, 6, $d->anggota_kode, 1, 0);
					$pdf->Cell(55, 6, $d->anggota_nama, 1, 0);
					$pdf->Cell(55, 6, 'Rp. ' . number_format($shu_simpanan), 1, 0, 'C');
					$pdf->Cell(55, 6, 'Rp. ' . number_format($shu_pinjaman), 1, 0, 'C');
					$pdf->Cell(55, 6, 'Rp. ' . number_format($total), 1, 1, 'C');

				endforeach;
				$pdf->SetFont('times', 'B', 12);
				$pdf->Cell(220, 6, 'Grand Total', 1, 0, 'C');
				$pdf->Cell(55, 6, 'Rp. ' . number_format($grand_total), 1, 0, 'C');
				$pdf->SetFont('times', '', 12);
				$pdf->ln(10);
				$pdf->Cell(270, 4, 'Kendal, ' . tanggal(date('Y-m-d')), 0, 0, 'R');
				$pdf->ln(30);
				$pdf->Cell(250, 5, 'Bendahara', 0, 0, 'R');
			} else {
				$pdf->Cell(282, 6, 'Data tidak ditemukan', 1, 1, 'C');
			}
			$pdf->Output('Laporan SHU Perorangan.pdf', 'I');
		} else {
			$data['laporan']	= 'active';
			$data['lshup']		= 'active';
			$data['body']		= 'page/lap_shup';
	...
	public function bukti_angsuran($aksi)
	{
		$data 		= $this->data->angsuran_detail($aksi)->row();

		$pdf 	= new FPDF('P', 'mm', 'A4');
		$pdf->AddPage();
		$pdf->Image(base_url('src/img/logo.png'), 10, 5, 25);
		$pdf->SetTitle('Cetak Bukti Pembayaran Angsuran');
		$pdf->SetFont('times', 'B', 16);
		$pdf->Cell(190, 5, 'Koperasi Arofah Kendal', 0, 1, 'C');
		$pdf->SetFont('times', '', 12);
		$pdf->Cell(190, 5, 'Jl. Sikopek Tengah No. 1 , Plantaran Kaliwungu Selatan, Kendal', 0, 1, 'C');
		...
		$pdf->SetFont('times', 'B', 14);
		$pdf->Cell(190, 5, 'Bukti Pembayaran Angsuran Pinjaman', 0, 1, 'C');
		...
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Kode Pinjaman', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->pinjaman_kode, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Nama Anggota', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->anggota_nama, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Angsuran Ke', 0, 0);
		$pdf->Cell(110, 5, ': ' . $data->angsuran_ke, 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Tanggal Pembayaran', 0, 0);
		$pdf->Cell(110, 5, ': ' . tanggal(date('Y-m-d', strtotime($data->angsuran_waktu))), 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Jumlah', 0, 0);
		$pdf->Cell(110, 5, ': Rp.' . number_format($data->angsuran_jumlah), 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Denda', 0, 0);
		$pdf->Cell(110, 5, ': Rp.' . number_format($data->angsuran_denda), 0, 1);
		$pdf->Cell(60, 5, '', 0, 0);
		$pdf->Cell(40, 5, 'Status Pembayaran', 0, 0);
		$bulan = explode('-', date('Y-m-d', strtotime($data->angsuran_waktu)));
		$pdf->Cell(110, 5, $bulan[2] > 10 ? ': Terlambat' : ': Lunas', 0, 1);
		$pdf->ln(10);
		$pdf->Cell(190, 4, 'Kendal, ' . tanggal(date('Y-m-d')), 0, 0, 'C');
		$pdf->ln(20);
		$pdf->Cell(65, 5, 'Anggota', 0, 0, 'R');
